...
instance (Selectable a, Selectable b) => Selectable (MapView a b) where
...
instance (Selectable a, Selectable b) => Selectable (KeyValueView a b) where
    applySelection =
        drillWithSel $ \mi kv ->
            case (mi, kv) of
                (Just (0, sel), KVVV a b) ->
                    KVVK (applySelection (Just sel) a) b
                (Just (1, sel), KVVK a b) ->
                    KVVV a (applySelection (Just sel) b)
                (_, _) -> kv
